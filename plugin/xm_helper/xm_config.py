...
class XM_Config():
    ...
    def Load_Config():
        '''【读取并加载配置文件到变量】'''
        # 读取ini配置文件
        conf_folder_path = f'./plugin/{XM_Config.folder_name}/config/conf'
        ...
        with lock:
            for file_name in files_name:  # 遍历文件夹
                ...
                if ((os.path.splitext(file_name)[1]).lower() == '.ini') and os.path.isfile(f'{conf_folder_path}/{file_name}'):
                    ...
                    XM_Config.all_config[os.path.splitext(file_name)[0]] = dict(Config.Read_Config(file_path=f'{conf_folder_path}/{file_name}'))
...
XM_Config.folder_name=os.path.basename(os.path.dirname(__file__)) # 获取插件所在的文件夹名
